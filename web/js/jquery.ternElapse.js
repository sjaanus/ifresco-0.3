...
jQuery.fn.extend({
	...
	function (a) {
		...
		this.maskElapsor = this.find('.ternmaskElapsor').get(0);
		if(!this.maskElapsor) {
			this.maskElapsor = document.createElement('div');
			jQuery(this.maskElapsor).css({
				'display'	: 'none',
				'position'	: 'absolute',
				'clear'		: 'both',
				'overflow'	: 'hidden',
			...
			this.prepend(this.maskElapsor);
			jQuery(this.maskElapsor).addClass('ternmaskElapsor');
		}
		if(a.image || a.text) {
	...
	function (a) {
		...
		if(this.elap) {
			...
			this.maskElapsor.appendChild(this.elap);
	...
	hideElapsor :
	function () {
		this.maskElapsor = this.find('.ternmaskElapsor').get();
		if(this.maskElapsor) {
			jQuery(this.maskElapsor).css({
				'display' : 'none'
			})
